...
namespace TaiwanPetroLibrary.ViewModels
{
    public class cpViewModel : viewmodelBase
    {
        ...
        public IEnumerable<priceStorage> monitorCollections
        {
            set
            {
            ...
            get
            {
                return (from ps in currentCollections where ps.monitored orderby ps.id descending select ps).GroupBy(p => p.kind).Select(p => p.First());
            }
        }
        public IEnumerable<priceStorage> cpcCollections
        {
            set {
                NotifyPropertyChanged();
            }
            get
            {
                return (from ps in currentCollections where ps.brand == 0 select ps);
            }
        }
        public IEnumerable<priceStorage> fpccCollections
        {
            set
            {
                NotifyPropertyChanged();
            ...
            get
            {
                return (from ps in currentCollections where ps.brand == 1 select ps);
            }
        }
        public IEnumerable<priceStorage> savedfpccCollections
        {
            set
            {
                NotifyPropertyChanged();
            ...
            get
            {
                return (from ps in fpccCollections where ps.saved select ps);
            }
        }
        public IEnumerable<priceStorage> savedcpcCollections
        {
            set
            {
                NotifyPropertyChanged();
            ...
            get
            {
                return (from ps in cpcCollections where ps.saved select ps);
            }
        }
        public cpViewModel() {
            currentCollections = new ObservableCollection<priceStorage>();
            cpcCollections = new List<priceStorage>();
            fpccCollections = new List<priceStorage>();
            savedcpcCollections = new List<priceStorage>();
            savedfpccCollections = new List<priceStorage>();
        ...
        System.Collections.Generic.List<int> _productMonitor = new List<int>();
        public System.Collections.Generic.List<int> productMonitor
        {
            set
            {
        ...
        public chartPlot setupHistorical()
        {
            chartPlot cp = new chartPlot();
            cp.pm = new PlotModel();
            cp.da = new DateTimeAxis() { Position = AxisPosition.Bottom, Angle = 70 };
            cp.l = new LinearAxis() { Position = AxisPosition.Left };
            cp.s = new LineSeries();
            cp.s.Title = "儲存的價格";
            cp.s.Color = OxyColor.FromRgb(200, 200, 200);
            cp.pl = new LineSeries();
            cp.pl.Title = "價格";
            cp.pl.Color = OxyColor.FromRgb(212, 61, 61);
            cp.pl.StrokeThickness = 2;
            cp.pl.MarkerFill = OxyColor.FromRgb(200, 56, 56);
            cp.pl.MarkerType = MarkerType.Circle;
            cp.pl.MarkerSize = 5;
            cp.pl.BrokenLineColor = OxyColor.FromRgb(212, 61, 61);
            cp.pl.BrokenLineStyle = LineStyle.Dot;
            cp.pl.BrokenLineThickness = 2;
            cp.pl.Points.Add(new DataPoint(DateTimeAxis.ToDouble(new DateTime(0)), 0));
            cp.a = new LineSeries();
            cp.a.Color = OxyColor.FromRgb(76, 134, 203);
            cp.a.StrokeThickness = 1;
            cp.a.Title = "平均價";
            cp.a.BrokenLineColor = OxyColor.FromRgb(76, 134, 203);
            cp.a.BrokenLineStyle = LineStyle.Dot;
            cp.a.BrokenLineThickness = 1;
            cp.a.Points.Add(new DataPoint(0, 0));
            cp.pm.Series.Add(cp.pl);
            cp.pm.Series.Add(cp.a);
            cp.pm.Series.Add(cp.s);
            cp.pm.Axes.Add(cp.da);
            cp.pm.Axes.Add(cp.l);
            return cp;
        }
        public chartPlot historicalPrice(chartPlot cp, double predictPrice, bool predictpause, string productid, IProgress<ProgressReport> messenger)
        {
            ...
            cp.l.Maximum = maxAxis;
            cp.l.Minimum = minAxis;
            cp.da.Maximum = maxDate;
            cp.da.Minimum = minDate;
            cp.pl.Points.Clear();
            cp.a.Points.Clear();
            foreach (priceStorage ps in historical)
            {
                double d = DateTimeAxis.ToDouble(new DateTime(ps.datetick));
                cp.pl.Points.Add(new DataPoint(d, ps.price));
            }
            cp.a.Points.Add(new DataPoint(DateTimeAxis.ToDouble(new DateTime(historical.Min(x => x.datetick))), avgPrice));
            cp.a.Points.Add(new DataPoint(DateTimeAxis.ToDouble(new DateTime(historical.Max(x => x.datetick))), avgPrice));
            if (!predictpause)
            {
                cp.pl.Points.Add(DataPoint.Undefined);
                cp.pl.Points.Add(new DataPoint(maxDate, predictPrice));
                cp.a.Points.Add(DataPoint.Undefined);
                cp.a.Points.Add(new DataPoint(maxDate, avgPrice));
            }
            if (tempsaved.Any())
            {
                cp.s.Points.Add(new DataPoint(DateTimeAxis.ToDouble(new DateTime(tempsaved.First().datetick)), cp.pl.Points.Select(x => x.Y).Max()));
                cp.s.Points.Add(new DataPoint(DateTimeAxis.ToDouble(new DateTime(tempsaved.First().datetick)), cp.pl.Points.Where(x => !double.IsNaN(x.Y)).Select(x => x.Y).Min()));
            }
            else
            {
                cp.pm.Series.Remove(cp.s);
            }
            ...
            if (connectivity)
            {
                try
                {
                    ...
                    using (httpClient = new HttpClient(handler))
                    {
                        ...
                        string data = Portable.Text.Encoding.GetEncoding(950).GetString(byteData, 0, byteData.Length);
                        //httpClient.Dispose();
                        ...
                        if (soapUpdate)
                        {
                            ...
                            if (gassoap.loaded)
                            {
                                tempDB.Add(new priceStorage() {
                                    brand = typeDB.CPCgasohol.brand,
                                    price = gassoap.price,
                                    datetick = gassoap.date.Ticks,
                                    kind = typeDB.CPCgasohol.key,
                                    saved = false,
                                });
                        ...
                        foreach (Match hn in match)
                        {
                            ...
                            for (int i = 1; i <= products.Length; i++)
                            {
                                ...
                                priceStorage ps = new priceStorage() {
                                    brand = brand,
                                    datetick = DateTime.Parse(s.Substring(s.IndexOf("日") - 10, 10)).AddHours(Convert.ToDouble(s.Substring(s.IndexOf("時") - 1, 1))).Ticks,
                                    kind = kind,
                                    price = Convert.ToDouble(co[products[i - 1]].ToString()),
                                    saved = false
                                };
    ...
    public struct chartPlot
    {
        public LinearAxis l;
        public DateTimeAxis da;
        public LineSeries pl;
        public LineSeries a;
        public LineSeries s;
        public PlotModel pm;
    }
