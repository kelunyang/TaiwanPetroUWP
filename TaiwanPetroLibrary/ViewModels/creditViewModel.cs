...
namespace TaiwanPetroLibrary.ViewModels
{
    public class discountViewModel : viewmodelBase
    {
        ...
        {
            get {
                _xmlContent = XDocument.Parse(im.creditXML);
                return _xmlContent;
            }
            set
            {
                im.creditXML = value.ToString();
                _xmlContent = value;
                NotifyPropertyChanged();
            }
        }
        ...
        public async Task<bool> updateXML(IProgress<ProgressReport> messenger)
        {
            try
            {
