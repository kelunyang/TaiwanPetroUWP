...
namespace TaiwanPetroLibrary.ViewModels
{
    public class stationViewModel : viewmodelBase
    {
        ...
        public List<GeoPoint> infoPoints { set; get; }
        //public List<string> countryitems { get; set; }
        ...
        public GeoPoint _centerloc = new GeoPoint(25.021918, 121.535285);
        ...
        public GeoPoint _currentloc = new GeoPoint(25.021918, 121.535285);
        public GeoPoint currentloc
        {
            get { return new GeoPoint(_currentloc.Latitude, _currentloc.Longitude, "currentloc"); }
            set
            {
                _currentloc = value;
                NotifyPropertyChanged();
        ...
        GeoPoint _sfilterlocation = new GeoPoint(25.021918, 121.535285);
        public GeoPoint sfilterlocation
        {
            get
            {
                _sfilterlocation = im.sfilterrangeLocation;
                return new GeoPoint(_sfilterlocation.Latitude, _sfilterlocation.Longitude, "queryloc");
            }
            set
            {
        ...
        public bool sfiltergpsLocation
        {
            get
            {
                _sfiltercustomLocation = im.sfiltercustomLocation;
        ...
        public stationViewModel() 
        {
            ...
            //countryitems = new List<string>(40);
        ...
        public override async Task loadDB(string dbPath)
        {
            ...
            var citylist = (from city in stationDB orderby city.city select city.city).Distinct();
            countryitems.Clear();
            ...
            filters.Add(sfiltercountryEnable ? countryitems.ElementAt(sfiltercountry) : sfiltercustomLocation ? "指定位置方圓" + sfilterKilonmeter + "公里內" : "目前位置方圓" + sfilterKilonmeter + "公里內");
            filtercount = filters.Count();
            infoPoints = new List<GeoPoint>() { sfilterlocation, currentloc };
        }
        ...
        public async Task queryCityStation(IProgress<ProgressReport> messenger)
        {
            ...
            this.centerloc = this.currentloc;
        ...
        public async Task queryDistStation(GeoPoint loc, IProgress<ProgressReport> messenger)
        {
            ...
            this.centerloc = queryStations.Count > 0 ? ((stationStorage)mapStations.First()).coordinance : new GeoPoint(25.021918, 121.535285);
            messenger.Report(new ProgressReport() { progress = 100, progressMessage = "查詢完成", display=false });
        }
        ...
        public async Task saveStation(stationStorage s,IProgress<ProgressReport> messenger)
        {
            ...
            stationStorage station = await dbConn.Table<stationStorage>().Where(x => x.sid == s.sid).FirstAsync();
            station.favorite = !station.favorite;
            await dbConn.UpdateAsync(station);
