    ...
    sealed partial class App : Application
    {
        ...
        protected override async void OnLaunched(LaunchActivatedEventArgs e)
        {
            if(AppSettings.Contains("version"))
            {
                ...
                {
                    IEnumerable<TaiwanPetroLibrary.Models.configSet> updatedList = from cs in im.configList where cs.version > AppSettings.GetValueOrDefault("version", DateTime.Now) select cs;
            ...
            {
                foreach(TaiwanPetroLibrary.Models.configSet cs in im.configList)
                {
                    cs.exeValue();
                }
        ...
        {
            var assfile = await StorageFile.GetFileFromApplicationUriAsync(new Uri("ms-appx:///Assets/"+filename));
